...
def get_active_users(post_id, board_id):
    queer =  g.db.query(User).filter(User.id.in_([x for x in g.db.query(Comment.author_id)
                                                        .filter(Comment.board_id == board_id)
                                                        .filter(Comment.parent_submission == post_id)
                                                        .filter(Comment.deleted_utc == 0)
                                                        .filter(Comment.is_banned == False)
...
"""
    ...
    queer1 =  g.db.query(Comment.author_id)
                    .filter(Comment.board_id == board_id)
                    .filter(Comment.parent_submission == post_id)
                    .filter(Comment.deleted_utc == 0)
                    .filter(Comment.is_banned == False)
    ...
    most_active = queer1.join(Comment.author_id)
                        .order_by(desc(Comment.author_id))
                        .limit(3)
                        .count()
    ## group uids
    for i in range(5):
        if i == 0:
            uids.append(first = queer1.order_by(desc(Comment.created_utc)).first().id)

        elif i > 0 and i < 4:
            [uids.append(x.id) for x in most_active]

        elif i == 4:
            last = queer1.order_by(desc(Comment.created_utc)).last()

    output =  g.db.query(User).filter(User.id.in_(uids)).all()

    return output
"""
def get_guild_members(bid, limit=25, page=1):
    #uids = g.db.query(Subscription.user_id).filter_by(board_id=bid,is_active=True,show_membership=True).all()
    #print(uids)
    ...
    #print(f"members query result{members}")
    members = [x.json_core for x in members]
