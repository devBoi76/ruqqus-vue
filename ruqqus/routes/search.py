...
def search(v, search_type="posts"):
    ...
    if query.startswith("+"):
        ...
        return {"html":lambda:render_template("search_boards.html",
                ...
                "vue":lambda:jsonify({"status_code": 200, "results":[x.json for x in boards]}),
        ...
        return {"html":lambda:render_template("search_users.html",
                ...
                "vue":lambda:jsonify({"status_code": 200, "results":[x.json_core for x in users]}),
        ...
        return {"html":lambda:render_template("search.html",
                ...
                "vue":lambda:jsonify({"status_code": 200, "results":[x.json for x in posts]}),
                "api":lambda:jsonify({"data":[x.json for x in posts]})
                }


@app.route("/+<name>/search", methods=["GET"])
@app.route("/api/v1/guild/<name>/search", methods=["GET"])
@app.route("/api/vue/guild/<name>/search", methods=["GET"])
@auth_desired
def search_guild(name, v, search_type="posts"):
    ...
    if not b:
        b = get_board(name, graceful=True)
        #abort(404)
