...
def login_post():
    ...
    if not account:
        ...
        return jsonify({"status_code": 400, "error": "Invalid Credentials"})
    ...
    if request.form.get("password"):
        ...
        if now - int(request.form.get("time")) > 600:
            return jsonify({"status_code": 302, "url": "/login"})
    ...
    return jsonify({"status_code": 200,
                    "v": account.vue_json}) # direct(account.url)
...
def post_forgot():
    ...
    return {"msg": 'If the username and email matches an account, you will be sent a password reset email. You have ten minutes to complete the password reset process.',
            "i": random_image()}
    """return render_template("forgot_password.html",
...
def post_reset(v):
    ...
    if now - timestamp > 600:
        return {"title": "Password reset expired",
    ...
    return {"title": "Password reset successful",
