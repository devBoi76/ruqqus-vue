...
@app.route("/post_short/<base36id>/", methods=["GET"])
@app.route("/api/vue/post_short", methods=["GET"])
@app.route("/api/vue/post_short/<base36id>", methods=["GET"])
def incoming_post_shortlink(base36id=None):

    if not base36id:
        return {"status_code": 302, "url": "/"}
        #return redirect('/')

    if base36id == "robots.txt":
        return {"status_code": 302, "url": "/robots.txt"}
    ...
    return {"status_code": 302, "url": post.permalink}
...
@app.route("/api/vue/+<boardname>/post/<base36id>", methods=["GET"])
@app.route("/api/vue/+<boardname>/post/<base36id>/<anything>", methods=["GET"])
@auth_desired
@api("read")
def post_base36id(base36id, boardname=None, anything=None, v=None):
    ...
    if boardname and not boardname == board.name:
        return {"vue":lambda:{"status_code": 302, "url": post.permalink}}
        #return redirect(post.permalink)

    if board.is_banned and not (v and v.admin_level > 3):
        return {"vue":lambda:{"status_code": 401, "error": f"{board.name} has been banned.\n{board.ban_reason}"}}
    ...
    if post.over_18 and not (v and v.over_18) and not session_over18(board):
        ...
        return {"html":lambda:render_template("errors/nsfw.html",
                ...
                "vue":lambda:{"status_code": 451, "error":"Must be 18+ to view"},
    ...
    return {
        ...
        "vue":lambda:{"status_code": 200, "data":post.json},
...
@app.route("/api/vue/post/<base36id>", methods=["GET"])
@app.route("/api/vue/post/<base36id>/<anything>", methods=["GET"])
@auth_desired
@api("read")
def post_base36id_noboard(base36id, anything=None, v=None):
    print('making it to endpoint')
    post=get_post_with_comments(base36id, v=v, sort_type=request.args.get("sort","top"))
    print('gathered post and comments')

    #print(f"jsn = {post.json}")
    try:
        print('gathering active users')
        active_users = [x.json_core for x in get_active_users(post.id, post.board_id)]

        print(f'active_users : {active_users} ')
        post.set_active_users(active_users)
    ...
    return {"html": lambda: redirect(post.permalink),
            "api" : lambda: jsonify({"data": post.json}),
            "vue": lambda: jsonify(post.json)
            }
    #return redirect(post.permalink)
...
@app.route("/api/vue/submit", methods=["GET"])
@is_not_banned
...
def submit_get(v):
    ...
    return {"status_code": 200, "guild": b}


@app.route("/edit_post/<pid>", methods=["POST"])
@app.route("/api/vue/edit_post/<pid>", methods=["GET"])
@is_not_banned
@no_negative_balance("html")
@validate_formkey
def edit_post(pid, v):
    ...
    if not p.author_id == v.id:
        return {"status_code": 403, "error": f"You may not edit posts that are not your own."}
        #abort(403)

    if p.is_banned:
        return {"status_code": 403, "error": f"{p.name} has been banned.\n{p.ban_reason}"}
        #abort(403)

    if p.board.has_ban(v):
        return {"status_code": 403, "error": "You have been banned from this Guild."}
        #abort(403)
...
@app.route("/api/vue/submit/title", methods=["GET"])
...
def get_post_title(v):
    ...
    if not url:
        return {"status_code": 400, "error": "No URL was provided."}, 400
        #return abort(400)
    ...
    try:
        ...
        return {"status_code": 400, "error": "Could not reach page"}, 400
        #return jsonify({"error": "Could not reach page"}), 400


    if not x.status_code == 200:
        return {"status_code": x.status_code, "error": f"Page returned {x.status_code}"}, x.status_code
    ...
    try:
        ...
        return jsonify({"status_code": 400,
                        "error": f"Could not find a title"})
...
def submit_post(v):
    ...
    #                            )
        ...
        return {"html": lambda: (render_template("submit.html",
                ...
                "vue": lambda: ({"error": "250 character limit for titles"}, 400),
    ...
    if not (parsed_url.scheme and parsed_url.netloc) and not request.form.get(
        ...
        return {"html": lambda: (render_template("submit.html",
                ...
                "vue": lambda: ({"error": "`url` or `body` parameter required."}, 400),
    ...
    if dup:
        return {"vue": lambda: {"status_code": 302,
                                "url": dup.permalink}
                                }
    ...
    if domain_obj:
        if not domain_obj.can_submit:
            ...
            return {"html": lambda: (render_template("submit.html",
                    ...
                    "vue": lambda: ({"error": BAN_REASONS[domain_obj.reason]}, 400),
    ...
    if not board:
        ...
        return {"html": lambda: (render_template("submit.html",
                ...
                "vue": lambda: (jsonify({"error": f"403 Not Authorized - You are exiled from +{board.name}"}), 403),
    ...
    if board.is_banned:
        ...
        return {"html": lambda: (render_template("submit.html",
                ...
                "vue": lambda: (jsonify({"error": f"403 Forbidden - +{board.name} has been banned."}), 403),
                "api": lambda: (jsonify({"error": f"403 Forbidden - +{board.name} has been banned."}), 403)
                }
    ...
    if (board.restricted_posting or board.is_private) and not (
        ...
        return {"html": lambda: (render_template("submit.html",
                ...
                "vue": lambda: (jsonfiy({"error": f"403 Not Authorized - You are not an approved contributor for +{board.name}"}), 403),
    ...
    if max(len(similar_urls), len(similar_posts)) >= threshold:
        ...
        for post in similar_posts + similar_urls:
            ...
            g.db.add(ma)
        g.db.commit()
        return {"vue": lambda: (jsonify({"status_code": 401,
                "error": text,
                "url": "/notifications"}), 401)
                }
        #return redirect("/notifications")
    ...
    if len(str(body)) > 10000:
        ...
        return {"html": lambda: (render_template("submit.html",
                ...
                "vue": lambda: ({"error": "10000 character limit for text body."}, 400),
    ...
    if len(url) > 2048:
        ...
        return {"html": lambda: (render_template("submit.html",
                ...
                "vue": lambda: ({"error": "2048 character limit for URLs."}, 400),
    ...
    for link in links:
        ...
        if badlink:
            if badlink.autoban:
                ...
                send_notification(v, text)
                v.ban(days=1, reason="spam")
                return {"vue": lambda: (jsonify({"error": text,"url": "/notifications"}),302)}
                #return redirect('/notifications')
            else:
                ...
                return {"html": lambda: (render_template("submit.html",
                        ...
                        "vue": lambda: ({"error": f"The link `{badlink.link}` is not allowed. Reason: {badlink.reason}"}, 400),
    ...
    if repost and request.values.get("no_repost"):
		...
		'vue': lambda:({"error":"This content has already been posted", "repost":repost.json}, 409)
	       }

    if request.files.get('file') and not v.can_submit_image:
        return {
                "vue": lambda: (jsonify({"error": "You cannot submit images yet!"}), 403),
                "api": lambda: (jsonify({"error": "You cannot submit images yet!"}), 403)
                }
    ...
    if request.files.get('file'):
        ...
        if request.content_length > 16 * 1024 * 1024 and not v.has_premium:
            g.db.rollback()
            return {"html": lambda: {abort(413)},
                    "vue": lambda: {"status_code": 413, "error": "Filesize is too large! Max Filesize is 16 MB."},
                    "api": lambda: {abort(413)}}
        ...
        if not file.content_type.startswith('image/'):
            return {"html": lambda: (render_template("submit.html",
                        ...
                        "vue": lambda: ({"error": f"Image files only"}, 400),
    ...
    return {"html": lambda: redirect(new_post.permalink),
            "vue": lambda: jsonify(new_post.json),
...
@app.route("/api/vue/delete_post/<pid>", methods=["POST"])
@auth_required
@api("delete")
@validate_formkey
def delete_post_pid(pid, v):
    ...
    if not post.author_id == v.id:
        {"vue": lambda: (jsonify({"error": "You cannot delete this post."}), 403)}

    if post.is_deleted:

        {"vue": lambda: (jsonify({"error": "You cannot delete this post."}), 401)}
...
@app.route("/api/vue/embed/post/<pid>", methods=["GET"])
def embed_post_pid(pid):

    post = get_post(pid)

    if post.is_banned or post.board.is_banned:
        return jsonify({"error": "This post is banned."}), 410
        #abort(410)
...
@app.route("/api/vue/toggle_post_nsfw/<pid>", methods=["POST"])
@is_not_banned
@api("update")
@validate_formkey
def toggle_post_nsfw(pid, v):
    ...
    if not post.author_id == v.id and not v.admin_level >= 3 and not mod:
        return jsonify({"error": "You cannot alter this post."}), 403

    if post.board.over_18 and post.over_18:
        return jsonify({"error": "Post must remain NSFW."}), 403
...
@app.route("/api/vue/toggle_post_nsfl/<pid>", methods=["POST"])
@is_not_banned
@api("update")
@validate_formkey
def toggle_post_nsfl(pid, v):
    ...
    if not post.author_id == v.id and not v.admin_level >= 3 and not mod:
        return {"vue": lambda: (jsonify({"error": "You cannot alter this post"}), 403)}

    if post.board.is_nsfl and post.is_nsfl:
        return  {"vue": lambda: (jsonify({"error": "This post must remain NSFL."}), 403)}
...
@app.route("/api/vue/retry_thumb/<pid>", methods=["POST"])
@is_not_banned
@validate_formkey
...
@app.route("/api/vue/save_post/<pid>", methods=["POST"])
@auth_required
@validate_formkey
def save_post(pid, v):
...
@app.route("/api/vue/unsave_post/<pid>", methods=["POST"])
@auth_required
@validate_formkey
