...
@app.route("/api/v1/comments/<p_id>")
@app.route("/api/vue/comments/<p_id>", methods=["GET"])
@app.route("/api/vue/post/<p_id>/comments", methods=["GET"])
@auth_desired
@api("read")
def post_pid_comments(p_id, v=None):
    post=get_post_with_comments(p_id, v=v, sort_type=request.args.get("sort","top"))

    if not post:
            return {"api": lambda: jsonify({"data": "No post exists"}),
                    "vue": lambda: (jsonify({"error": "No post exists"}), 404)
                    }

    if "replies" in post.json.keys():
        return {"api": lambda: jsonify({"data": post.json['replies']}),
                "vue": lambda: (jsonify({"results": post.json['replies']}), 200)
                }

    return {"api": lambda: jsonify({"data": []}),
            "vue": lambda: (jsonify({"results": []}), 200)
            }


@app.route("/api/v1/comment/<c_id>", methods=["GET"])
@app.route("/+<boardname>/post/<p_id>/<anything>/<c_id>", methods=["GET"])
@app.route("/api/vue/comment/<c_id>", methods=["GET"])
@app.route("/api/vue/post/<p_id>/comment/<c_id>", methods=["GET"])
@auth_desired
@api("read")
def post_pid_comment_cid(c_id, p_id=None, boardname=None, anything=None, v=None):
    ...
    if board.is_banned and not (v and v.admin_level > 3):
        return {'html': lambda: render_template("board_banned.html",
                                                v=v,
                                                b=board),
                'vue': lambda: {'error': f'+{board.name} is banned.'},
    ...
    if post.over_18 and not (
        ...
        return {'html': lambda: render_template("errors/nsfw.html",
                ...
                'vue': lambda: {'error': f'This content is not suitable for some users and situations.'},
...
@app.route("/api/vue/comment", methods=["POST"])
...
@app.route("/api/vue/comment/<cid>", methods=["PUT"])
@is_not_banned
@validate_formkey
@api("edit")
def edit_comment(cid, v):
    ...
    if bans:
        ...
        return {'html': lambda: render_template("comment_failed.html",
                ...
                'vue': lambda: ({'error': f'A blacklisted domain was used.'}, 400)
                }
...
@app.route("/api/vue/comment/<cid>", methods=["DELETE"])
@auth_required
@validate_formkey
@api("delete")
...
@app.route("/api/vue/comment/<cid>/pin", methods=["PUT"])
@auth_required
@is_guildmaster("content")
@api("guildmaster")
@validate_formkey
def mod_toggle_comment_pin(cid, board, v, bid=None):
    ...
    return {"html": lambda: jsonify({"html":html}),
            "api": lambda: ("", 204),
            "vue": lambda: ("", 204)}
