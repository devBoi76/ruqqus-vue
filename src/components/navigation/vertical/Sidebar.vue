				...
				<li v-for="(item, index) in section.items" :key="index" class="item">
					...
					<div v-if="!item.header">
						<router-link custom :to="item.route" v-slot="{ href, navigate, isActive }">
							<a :href="href" @click="navigate" class="relative block px-6 py-1.5 text-left transition duration-100" :class="isActive ? 'text-white font-medium bg-primary bg-opacity-40' : 'text-gray-700 hover:bg-gray-100'">
								<span v-if="!item.icon">
									<img
									:id="item.id"
									v-lazy="item.src"
								...
								<span v-else>
									<i class="far fa-fw mr-2" :class="[item.icon, isActive ? 'text-white' : 'text-gray-500']"></i>
