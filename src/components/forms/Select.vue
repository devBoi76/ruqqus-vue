<template>
  ...
  <Listbox as="div" v-model="selectedPerson">
    <div class="relative">
      ...
      <ListboxOptions as="div" class="absolute right-0 w-64 mt-4 py-2 origin-top-right bg-white divide-y rounded shadow-lg border focus:outline-none">
        <ListboxOption v-slot="{ active, selected }" as="div" v-for="person in people" :key="person" :value="person">
          <div :class="[active ? 'text-primary font-medium bg-gray-100' : 'text-gray-700','relative cursor-default select-none relative py-2 pl-10 pr-4',]">
            {{ person.name }}
