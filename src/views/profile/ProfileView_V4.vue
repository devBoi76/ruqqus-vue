		...
		<vue-scroll>
			<div class="grid grid-cols-12 xl:grid-cols-8">
				<div class="col-span-full xl:col-start-2 xl:col-end-8 flex gap-6 md:p-4 xl:p-0">
					...
					<div class="flex flex-col flex-grow bg-gray-900 bg-opacity-93 md:p-4 gap-4">

						<div class="flex gap-4">

							<div v-if="!loading && u && !u.is_banned" class="w-full p-4 bg-black bg-opacity-40 rounded-sm" :class="{'lg:w-72':u.banner_url}">
							...
							<!-- User Banner -->
							<div v-if="u.banner_url" class="hidden lg:flex flex-grow h-56 bg-cover bg-center rounded-sm" :style="{ 'background-image': 'url(' + u.banner_url + ')' }">
							</div>
							<!-- End User Banner -->
						...
						<div class="flex flex-grow gap-4">
							...
							<div class="w-72 hidden lg:block flex-shrink-0">

								<!-- Sidebar -->
								<div v-if="!loading && u && !u.is_banned" class="space-y-4">
									<div class="bg-black bg-opacity-40 rounded-sm">
										...
										<div class="px-4 py-3 space-y-4">
											...
											<div class="flex justify-around px-2 py-3 bg-black bg-opacity-20 rounded-sm">
												...
												<div class="text-center">
													...
													<div class="text-xs text-gray-400">Tipped</div>
												</div>
											</div>
										</div>
									</div>
										...
										<div class="px-4 py-3">
											<div v-if="u.badges.length">
												<ul class="flex flex-wrap -mx-1 mb-0 overflow-hidden">
													<li v-for="badge in u.badges" :key="badge.name" class="my-1 px-1 w-1/5 overflow-hidden">
							...
							<div class="flex flex-col flex-grow">
								...
								<div v-if="!loading" class="flex flex-col flex-grow">

									<!-- Tabs -->
									<div v-if="u && !u.is_private && !u.is_banned" class="flex flex-shrink-0 items-end w-full h-12 mb-4 px-4 bg-opacity-40 bg-black rounded-sm mb-4">
									...
									<div class="flex gap-4">
										...
										<div v-if="u.is_private" class="flex flex-grow items-center justify-center p-12 bg-black bg-opacity-40 rounded-sm">
...
export default {
	...
	data() {
		return {
			...
			frame: 'https://i.ibb.co/PZ4rj8Z/6e1b5f5977036a189465f5455f2c54722c12883d.png',
			username: null,
			loading: true,
			errored: false,
			u: {},
	...
	computed: {
		...mapState("persist", ["v"]),
		repStyle() {
			const rep = (this.u.post_rep + this.u.comment_rep);
	...
	watch: {
		'$route.params.username': { // get guild info and posts if guild changes
			handler() {
				this.getAccountInfo()
			},
			immediate: true
